...
{
  struct ParameterizationData : public igl::Serializable
  {
    ...
    Eigen::VectorXi fixedValues;   //translation fixed values
    ...
    void InitSerialization() {
      this->Add(N,"N");
      this->Add(d,"d");
      this->Add(symmFunc,"symmFunc");
      this->Add(vertexTrans2CutMat,"vertexTrans2CutMat");
      this->Add(constraintMat,"constraintMat");
      this->Add(symmMat,"symmMat");
      this->Add(intSpanMat,"intSpanMat");
      this->Add(constrainedVertices,"constrainedVertices");
      this->Add(integerVars,"integerVars");
      this->Add(face2cut,"face2cut");
      this->Add(fixedIndices,"fixedIndices");
      this->Add(fixedValues,"fixedValues");
      this->Add(vertexTrans2CutMatInteger,"vertexTrans2CutMatInteger");
      this->Add(constraintMatInteger,"constraintMatInteger");
      this->Add(symmMatInteger,"symmMatInteger");
      this->Add(intSpanMatInteger,"intSpanMatInteger");
      this->Add(singularIndices,"singularIndices");
     }
                                         ...
                                         Eigen::MatrixXi& cutF)
  {
    ...
    */
    ...
    for(auto item : vertexTrans2CutTripletsInteger){
      if(item.value() != 0) {
          cleanTripletsInteger.push_back(item);
          Triplet< double > tmp(item.row(), item.col(), (double) item.value());
          cleanTriplets.push_back(tmp);
      }
    }
    ...
    for(auto item : constTripletsInteger){
      if(item.value() != 0) {
        cleanTripletsInteger.push_back(item);
        Triplet< double > tmp(item.row(), item.col(), (double) item.value());
        cleanTriplets.push_back(tmp);
    ...
    pd.intSpanMat.conservativeResize(d * (wholeV.rows() + numTransitions), d * (wholeV.rows() + numTransitions));
    pd.intSpanMatInteger.conservativeResize(d * (wholeV.rows() + numTransitions), d * (wholeV.rows() + numTransitions));
    ...
    pd.intSpanMat.setFromTriplets(intSpanMatTriplets.begin(), intSpanMatTriplets.end());
    pd.intSpanMatInteger.setFromTriplets(intSpanMatTripletsInteger.begin(), intSpanMatTripletsInteger.end());    
    ...
    if (isSingular.sum()==0){  //no inner singular vertices; vertex 0 is set to (0....0)
      ...
      for (int j=0;j<pd.d;j++)
        pd.fixedIndices(j)=pd.d*firstSing+j;
    }
    pd.fixedValues.resize(pd.d);
    pd.fixedValues.setZero();

    //creating list of singular corners
    VectorXi singularIndices(pd.d*isSingular.sum());
    int counter=0;
    for (int i=0;i<isSingular.size();i++){
      if (isSingular(i))
        for (int j=0;j<pd.d;j++)
          singularIndices(counter++)=pd.d*i+j;
    }
    pd.singularIndices=singularIndices;

    cout<<"pd.fixedIndices: "<<pd.fixedIndices<<endl;
    cout<<"pd.fixedValues: "<<pd.fixedValues<<endl;
  }
}
